...
from .db import get_all_todos, get_some_todos, want_add_something, want_update_something, want_del_something 
...
def call_getsome(todo_id: int):
    ...
    if wyk is None:
        raise HTTPException(status_code=404, detail="Can't find what u want")  
    return wyk

@router.post("/todos", response_model=Todo)
def call_addsome(todo: Todo):
    want_add_something(todo)   
    return todo
...
def call_del(todo_id: int):
    ...
    return {"message": "Mission Accomplished!"}  